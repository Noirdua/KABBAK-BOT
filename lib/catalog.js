...
const AUTH_FAILURE_CHOICE = Object.freeze({
  name: 'API key rejected — run /kabbak api login',
  value: 'api login',
});
const NETWORK_FAILURE_CHOICE = Object.freeze({
  name: 'KABBAK API unreachable — try again later',
...
async function cached(name, loader) {
  ...
  try {
    ...
    entry.ttl = 5000;
    entry.fetchedAt = now;
    if (error?.authFailure || error?.networkFailure) {
      entry.error = error;
      throw error;
    }
    entry.error = null;
...
async function suggest(optionName, query, linked = {}, { autocomplete = false } = {}) {
  try {
    return await suggestChoices(optionName, query, linked);
  } catch (error) {
    if (error?.authFailure) {
      return autocomplete ? [AUTH_FAILURE_CHOICE] : [];
    }
    if (error?.networkFailure) {
      return autocomplete ? [NETWORK_FAILURE_CHOICE] : [];
    }
    throw error;
  }
}

async function suggestChoices(optionName, query, linked = {}) {
  ...
  if (optionName === 'template') {
    const { listTemplates } = require('./spread-templates');
    return filterMapChoices(
      listTemplates(),
      query,
      (template, q) => String(template.id || '').toLowerCase().includes(q)
        || String(template.name || '').toLowerCase().includes(q),
      (template) => ({
        name: `${template.name} (${template.kind})`,
        value: String(template.id),
      }),
      (template) => [template.id, template.name, template.kind]
    );
  }
  ...
  if (optionName === 'deck') {
    // Tarot features are tarot-only; I Ching and playing-card decks are
    // excluded so card image resolution never targets a non-tarot deck.
