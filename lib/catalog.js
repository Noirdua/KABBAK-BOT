...
async function suggest(optionName, query, linked = {}) {
  ...
  if (optionName === 'deck') {
    // Spreads are tarot-only; I Ching decks are excluded so card image
    // resolution never targets a hexagram deck.
    const decks = (await getDeckOptions())
      .filter((deck) => String(deck?.system || 'tarot').trim().toLowerCase() !== 'iching');
    return filterMapChoices(
      decks,
      query,
