...
// The 0.22 API is used throughout (new Jimp(w,h,color), Jimp.loadFont, Jimp.MIME_*).
// jimp 1.x exposes a different, ESM-style API, so fail with an actionable message
// instead of a cryptic "Jimp is not a constructor".
function assertJimp() {
  if (typeof Jimp === 'function' && typeof Jimp.loadFont === 'function' && typeof Jimp.measureText === 'function') {
    return;
  }
  let version = 'unknown';
  try {
    version = require('jimp/package.json').version;
  } catch (_error) {
    // ignore
  }
  throw new Error(`Tarot stitching needs jimp 0.22.x, but ${version} is installed. Run: npm install jimp@0.22.12`);
}
