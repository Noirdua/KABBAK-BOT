...
const { stitchSpreadImages, listPrintTemplates } = require('./spread-stitch');
...
async function handleTarot(args = {}) {
  ...
  try {
    ...
    if (action === 'spreads') {
      ...
      const spreads = Array.isArray(payload) ? payload : (Array.isArray(payload?.spreads) ? payload.spreads : []);
      const list = spreads.map((s) => {
        const positions = Array.isArray(s.positions) ? s.positions.length : (Number(s.cardCount) || 0);
        return `**${s.id}** — ${s.label || s.name || s.id} (${positions || '?'} cards)`;
      }).join('\n');
      return card('Available Spreads', list || 'None');
    }
    if (action === 'templates') {
      const list = listPrintTemplates().map((template) => (
        `**${template.id}** — ${template.name} (${template.kind}, ${template.source})`
      )).join('\n');
      return card(
        'Spread templates',
        `${list || 'None'}\n\nDrop \`.svg\` or \`.json\` files in \`templates/\` or \`storage/templates/\`. Use \`data-kabbak="card"\` image slots (and optional title/label/name/meaning text). \`atelier\` is the built-in print.`
      );
    }
    if (action === 'pull' || action === 'draw') {
      ...
      if (shouldStitch) {
        const buffer = await stitchSpreadImages(drawnCards, {
          spreadId,
          title: spread.label || spread.name || spreadId,
      ...
      if (spread.description) {
        cardEmbeds.unshift(card(`Tarot: ${spread.label || spread.name || spreadId}`, spread.description));
      }
      return cardEmbeds;
    }
...
function formatNatalPoint(point) {
  if (!point) return '--';
  const rx = point.retrograde ? ' Rx' : '';
  const house = point.house ? ` H${point.house}` : '';
  return `${point.symbol || ''} ${point.sign?.symbol || ''} ${point.sign?.name || ''} ${point.degreeLabel || ''}${rx}${house}`.trim();
}

async function handleNatal(args = {}) {
  try {
    const date = String(args.date || args.datetime || '').trim();
    if (!date) {
      return card('Natal Chart', `Provide a birth date, e.g. ${cmd(args, 'natal 1979-03-14 time:11:30 Los Angeles')}`);
    }
    const location = await resolveLocation(args);
    const params = {
    ...
    if (/T\d{2}:\d{2}/.test(date) || date.includes('Z')) {
      params.datetime = date;
    } else {
      params.date = date;
    }
    if (args.time) params.time = String(args.time);
    if (args.timeUnknown) params.timeUnknown = '1';
    const data = await apiGet('/astrology/natal', params, args.apiKey);
    const chart = data.data || data;
    const fields = [
      { name: 'Sun', value: formatNatalPoint(chart.points?.sun), inline: true },
      { name: 'Moon', value: formatNatalPoint(chart.points?.moon), inline: true },
      { name: 'Rising', value: formatNatalPoint(chart.points?.ascendant) || (chart.birth?.timeUnknown ? 'Needs birth time' : '--'), inline: true },
    ];
    (chart.planets || []).slice(0, 8).forEach((planet) => {
      fields.push({
        name: planet.name || planet.id,
        value: formatNatalPoint(planet),
        inline: true,
      });
    });
    const aspects = (chart.aspects || []).slice(0, 6).map((aspect) => (
      `${aspect.from?.symbol || aspect.from?.name} ${aspect.glyph || ''} ${aspect.to?.symbol || aspect.to?.name} (${aspect.orb}°)`
    ));
    if (aspects.length) {
      fields.push({ name: 'Aspects', value: aspects.join('\n'), inline: false });
    }
    const notes = (chart.notes || []).join(' ');
    return card(
      'Natal Chart',
      `${chart.birth?.date || date}${chart.birth?.time ? ` ${chart.birth.time}` : ''}\nLocation: ${describeLocation(location, args)}${notes ? `\n${notes}` : ''}`,
      fields
    );
  } catch (e) {
    return card('Natal Error', e.message);
  }
}
...
async function handleGematria(args = {}) {
  try {
    ...
    if (!Number.isFinite(value) || value < 0) {
      return card('Gematria', `Provide a non-negative integer value, e.g. ${cmd(args, 'gematria value:111')} (optionally language:hebrew method:gadol)`);
    }
    const language = String(args.language || '').trim().toLowerCase();
    const method = String(args.method || '').trim().toLowerCase();
    const query = { value: String(Math.trunc(value)) };
    if (language === 'hebrew' || language === 'greek') {
      query.language = language;
      if (method) query.method = method;
    }
    const data = await apiGet('/gematria/words', query, args.apiKey);
    const payload = data.data || data;
    const matches = Array.isArray(payload?.matches) ? payload.matches : [];
    const payloadLanguage = String(payload?.language || 'english').toLowerCase();
    const payloadMethod = String(payload?.method || payload?.meta?.method || '').trim();
    const dictionary = payloadLanguage === 'hebrew' || payloadLanguage === 'greek';
    const titleSuffix = dictionary
      ? ` (${payloadLanguage}${payloadMethod ? ` · ${payloadMethod}` : ''})`
      : '';
    if (!matches.length) {
      return card(`Gematria: ${Math.trunc(value)}${titleSuffix}`, dictionary
        ? 'No dictionary words match this value and method.'
        : 'No words in the index carry this value.', [
        { name: 'Value', value: String(Math.trunc(value)), inline: true },
      ]);
    }
    const lines = matches.slice(0, 15).map((m) => {
      const name = m.word || m.name || m.id;
      const transliteration = m.transliteration ? ` *(${m.transliteration})*` : '';
      ...
      return `**${name}**${transliteration}${definition}${ciphers}`;
    }).join('\n');
    const fields = [{ name: 'Matches', value: String(matches.length), inline: true }];
    if (payload?.meta?.source) {
      fields.push({ name: 'Source', value: String(payload.meta.source), inline: true });
    }
    return card(`Gematria: ${Math.trunc(value)}${titleSuffix}`, lines || 'No matches', fields);
  } catch (e) {
...
async function handleDecks(apiKey = API_KEY) {
  try {
    const data = await apiGet('/decks/options', {}, apiKey);
    const payload = data.data || data;
    const decks = Array.isArray(payload?.decks)
      ? payload.decks
      : (Array.isArray(payload?.data?.decks) ? payload.data.decks : []);
    const labelFor = (deck) => {
      const system = String(deck.system || 'tarot').trim().toLowerCase();
      return system && system !== 'tarot' ? `${deck.label || deck.name || ''} (${system})` : (deck.label || deck.name || '');
    };
    const list = decks.map((d) => `**${d.id}** — ${labelFor(d)}`).join('\n');
    return card('Card Decks', list || 'No decks registered');
  } catch (e) {
...
function quizCardFromQuestion(q, index, total) {
  const options = Array.isArray(q?.options) ? q.options : [];
  const quizId = crypto.randomBytes(8).toString('hex');
  const buttons = options.slice(0, 5).map((option, optionIndex) => ({
    id: `quiz:${quizId}:${optionIndex}`,
    label: truncate(`${optionIndex + 1}. ${option || `Option ${optionIndex + 1}`}`, 80),
  }));
  if (options.length) {
  ...
  return card(
    total > 1 ? `Quiz ${index + 1}/${total}` : 'Quiz Question',
...
async function startQuiz(ctx = {}, categoryId, count) {
  try {
    const n = quizCount(count);
    const params = { includeAnswer: 'true', count: n };
    if (categoryId) params.categoryId = categoryId;
    const data = await apiGet('/quiz/session', params, ctx.apiKey);
    const payload = data.data || data;
    const questions = Array.isArray(payload.questions) ? payload.questions : [];
    if (!questions.length) {
      return card('Quiz', 'No questions available.');
    }
    return questions.map((question, index) => quizCardFromQuestion(question, index, questions.length));
  } catch (e) {
...
function handleHelp(ctx = {}) {
  ...
  return card('KABBAK Commands', `Everything lives under ${cmd(ctx)}. Save your own API key once and the bot remembers it.`, [
    ...
    { name: 'Tarot', value: `${p('tarot draw [spread] [deck]')} — one image with cards and meanings (reversed:true to allow reversals; stitch:false for separate cards; template:atelier)\n${p('tarot card name: [deck]')} — look up one card with its image\n${p('tarot cards [query]')} — list/search cards\n${p('tarot spreads')} — list spreads\n${p('tarot templates')} — list print templates`, inline: false },
    { name: 'Time & location', value: `${p('now')} — astrological snapshot (type a city in location)\n${p('calendar')} — week events\n${p('natal date: [time:] [location]')} — natal chart (sun, moon, rising, planets)\n${p('location set')} — save a place`, inline: false },
    { name: 'Library', value: `${p('iching [number]')} — I-Ching hexagram\n${p('tattva [tattva]')} — Golden Dawn tattva\n${p('gematria value: [language:hebrew] [method:gadol]')} — reverse gematria lookup (English index, or Strong's Hebrew/Greek)\n${p('quiz [category] [count:5]')} — quiz round (1–5 questions, one message each)\n${p('search query: [source] [work]')} — full-verse text search\n${p('text sources|section')} — read passages from the library`, inline: false },
...
async function resolveNamedArgs(group, subcommand, args = {}) {
  ...
  if (next.template) next.template = await resolveChoice('template', next.template);
...
async function dispatch({ userId, group = '', subcommand = '', args = {}, prefix = '/kabbak' } = {}) {
  ...
  if (group === 'tarot') {
    switch (subcommand) {
      ...
      case 'templates':
        return handleTarot({ action: 'templates', ...ctx });
      default:
        return card('Tarot', 'Pick a subcommand: draw, card, cards, spreads, or templates.');
    }
  }
  if (group === 'text') {
  ...
  switch (subcommand) {
    ...
    case 'gematria':
      return handleGematria({ value: args.value, language: args.language, method: args.method, ...ctx });
    case 'search':
      return handleSearch(args.query || '', ctx);
    case 'quiz':
      return startQuiz(ctx, args.category, args.count);
    default:
