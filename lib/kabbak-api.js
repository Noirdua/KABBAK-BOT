...
function tagApiError(error, { status, code, personal } = {}) {
  error.httpStatus = Number.isInteger(status) ? status : undefined;
  error.authFailure = status === 401 || status === 403;
  error.networkFailure = code === 'ECONNREFUSED' || code === 'ENOTFOUND' || code === 'ETIMEDOUT' || code === 'ECONNABORTED';
  error.personalKey = Boolean(personal);
  return error;
}
...
function friendlyApiError(err, usedPersonalKey) {
  ...
  if (status === 401 || status === 403) {
    return tagApiError(new Error(usedPersonalKey
      ? 'Your saved API key was rejected. Update it with **/kabbak api login**.'
      : 'The bot could not authenticate with KABBAK. Save your own key with **/kabbak api login**.'), { status, personal: usedPersonalKey });
  }
  if (status === 429) {
    return tagApiError(new Error('KABBAK is rate-limiting requests. Wait a moment and try again.'), { status });
  }
  if (code === 'ECONNREFUSED' || code === 'ENOTFOUND' || code === 'ETIMEDOUT' || code === 'ECONNABORTED') {
    return tagApiError(new Error('Could not reach the KABBAK API right now. Try again in a moment.'), { code });
  }
  const msg = err.response?.data?.message || err.response?.data?.error || err.message;
  return tagApiError(new Error(msg || 'API request failed.'), { status });
}
